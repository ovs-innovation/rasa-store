...
const ProductImageGallery = ({ images, productTitle = "Product", buttons }) => {
  ...
  const displayImages =
    validMedia.length > 0 ? validMedia : [PRODUCT_PLACEHOLDER];

  const imagesSignature = displayImages.join("\u0001");
  ...
  useEffect(() => {
    setActiveIndex((current) =>
      current >= displayImages.length ? 0 : current
    );
        ...
        <div className="flex lg:flex-col flex-row gap-3 order-2 lg:order-1 overflow-x-auto lg:overflow-x-visible lg:overflow-y-auto max-h-[560px] pb-2 lg:pb-0 scrollbar-thin">
          {displayImages.map((mediaUrl, index) => (
            <button
              ...
              className={`flex-shrink-0 relative w-16 h-16 lg:w-20 lg:h-20 rounded-xl border-2 overflow-hidden transition-all duration-300 transform ${
                index === activeIndex
